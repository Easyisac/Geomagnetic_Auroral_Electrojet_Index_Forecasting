...
def train_model(model, data, validation, dir, model_name):
    ...
    es = callbacks.EarlyStopping(monitor='val_loss', mode='min', patience=25, verbose=1)
    log_dir = "{}/logs/fit/".format(dir) + datetime.now().strftime("%Y%m%d-%H%M%S")
    tensorboard = tensorflow.keras.callbacks.TensorBoard(log_dir=log_dir, update_freq="epoch")
